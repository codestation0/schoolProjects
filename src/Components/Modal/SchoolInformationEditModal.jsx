...
const SchoolInformationEditModal = ({
  refetch,
  closeModalEdit,
  isOpenEdit,
  id,
  ...
  const { data: defaultInfo = {}, refetch: defaultRefetch } = useQuery({
    queryKey: ["single-school-information", id],
    queryFn: async () => {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/single-school-information/${id}`
      );
      return res.data;
    },
  });
  ...
  const modalHandlerEdit = async () => {
    await axios.patch(
      `${import.meta.env.VITE_BASE_URL}/update-school-information/${id}`,
      formData,
      {
        headers: {
...
export default SchoolInformationEditModal;
