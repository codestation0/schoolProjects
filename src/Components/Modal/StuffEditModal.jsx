...
const StuffEditModal = ({ refetch, closeModalEdit, isOpenEdit, id }) => {
  ...
  const { data: defaultInfo = {}, refetch: defaultRefetch } = useQuery({
    queryKey: ["single-stuff", id],
    queryFn: async () => {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/single-stuff/${id}`
      );
      return res.data;
    },
  });
  ...
  const modalHandlerEdit = async () => {
    await axios.patch(
      `${import.meta.env.VITE_BASE_URL}/update-stuff/${id}`,
      formData,
      {
        headers: {
