...
const HeadmasterBaniModal = ({ refetch, closeModalEdit, isOpenEdit, id }) => {
  const [description, setDescription] = useState("");
  const inputRef = useRef();

  const { data: defaultDesc = {}, refetch: defaultRefetch } = useQuery({
    queryKey: ["single-headmasterbani", id],
    queryFn: async () => {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/single-headmasterbani/${id}`
      );
  ...
  const modalHandlerEdit = async (id) => {
    await axios.patch(
      `${import.meta.env.VITE_BASE_URL}/update-headmasterbani/${id}`,
      {
        description,
      }
    );
...
export default HeadmasterBaniModal;
