...
const CommunicationModal = ({ refetch, closeModalEdit, isOpenEdit, id }) => {
  ...
  const { data: defaultInfo = {}, refetch: defaultRefetch } = useQuery({
    queryKey: ["single-communication", id],
    queryFn: async () => {
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL}/single-communication/${id}`
      );
      return res.data;
    },
  });
  ...
  const modalHandlerEdit = async () => {
    await axios.patch(
      `${import.meta.env.VITE_BASE_URL}/update-communication/${id}`,
      formData
    );
    closeModalEdit();
  };
                  ...
                  <div className="max-w-lg mx-auto mt-8 ">
                    <form className="space-y-6 bg-white p-5 rounded-md shadow-md">
                      <div>
                        ...
                        <input
                          defaultValue={defaultInfo.schoolName}
                          type="text"
                          id="schoolName"
                          name="schoolName"
                      ...
                      <div>
                        ...
                        <input
                          defaultValue={defaultInfo.email}
                          type="email"
                          id="email"
                          name="email"
                          onChange={handleChange}
