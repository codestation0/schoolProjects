...
import SchoolInformationEditModal from "../Modal/SchoolInformationEditModal";
const SchoolInfoRow = ({ schoolinfo, refetch }) => {
  const { _id, name, image, code } = schoolinfo;
  const [isOpen, setIsOpen] = useState(false);
  const [isEditOpen, setEditIsOpen] = useState(false);

  const handleDelete = async (id) => {
    const res = await axios.delete(
      `${import.meta.env.VITE_BASE_URL}/delete-school-information/${id}`
    );
