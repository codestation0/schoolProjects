...
import ImportantLinkModal from "../Modal/ImportantLinkModal";

const ImportantLinkRow = ({ link = {}, refetch }) => {
  const { _id, name, http } = link;
  ...
  const handleDelete = async (id) => {
    const res = await axios.delete(
      `${import.meta.env.VITE_BASE_URL}/delete-importantlinks/${id}`
    );
  ...
  return (
    <tr className="odd:bg-zinc-300 hover:bg-zinc-400 hover:text-white transition">
      ...
      <td className="px-6 py-4 whitespace-nowrap">
        <div className="text-sm text-gray-900">{http}</div>
      </td>
...
export default ImportantLinkRow;
